...
/** The default theme color — Artemis neon cyan. */
export const DEFAULT_THEME_COLOR = '#00D9FF'
...
export const THEME_COLOR_PRESETS: readonly ThemeColorPreset[] = Object.freeze([
  { id: 'terminal', labelKey: 'edex.preset.analyzed', color: '#00D9FF' },
...
export function tokenOverridesFor(palette: EdexPalette): ThemeTokenOverrides {
  return {
    // Backgrounds match the shell panel tone so the workspace reads as the same surface.
    '--dsw-alias-bg-base': both(palette.panel2),
    '--dsw-alias-bg-layer-1': both(palette.panel2),
    '--dsw-alias-bg-layer-2': both(palette.panel2),
    '--dsw-alias-bg-overlay': both(palette.panel2),
    ...
    '--dsw-specific-sidebar-fill': both(palette.panel2),
    '--dsw-specific-input-major': both(palette.panel2),
  }
}
